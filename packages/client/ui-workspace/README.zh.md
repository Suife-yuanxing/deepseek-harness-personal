...
侧边栏浏览器在列表持有联邦时于会话树上方渲染管理块：叠层文件夹行带成员 tooltip、计数胶囊，以及含重命名（对联邦标题集做重名校验）与删除（确认对话框言明非破坏性语义——文件夹、工作区与会话记录保留，现有联合会话继续可用，仅停止新建认领）的悬停菜单。管理块不受灰度开关约束：开关关闭前创建的联邦仍可管理。列表行标记存在性探针缺失：宿主 list 处理器对成员路径做 stat 并返回 `missingMembers`（联邦）／`missing`（工作区），成员目录消失即在菜单行、管理行与 tooltip 中显示「已失效」标记；创建面板将此类工作区锁定在成员集之外并内联说明原因。未探针的投影（字段缺省）表示「未知」，不渲染标记。
...
- **联邦成员创建后不可变**：重命名与删除是仅有的管理动词且没有重排席位（v1 顺序即创建序）；联邦没有聚合会话视图，会话中途增删根在设计上即不在范围内。
