...
当部署的灰度开关（`workspace.list.federatedWorkspacesEnabled`）开启时，选择菜单在普通工作区之后列出持久化联邦：每行携带内联叠层文件夹图形、全圆角胶囊成员计数（×N），以及悬停 tooltip 逐一列出成员 basename 并为主根标注「主」。选中一项即经注入载体认领该联邦身份——宿主解析主根 cwd、成员根与主根挂靠——并打开返回的会话。**新建联合工作区…** 动作拉起创建面板：以勾选行形式列出已注册工作区（勾选顺序即成员顺序），每个已勾选行提供「设为主文件夹」ghost 按钮；另有「添加文件夹…」行——仅当所在表面的目录流程孔被占用时渲染——以面板为 owner 运行本表面目录流程，把选中的宿主目录追加为自由成员（自由成员不可设主：联合会话附着在主目录对应的已注册工作区上，其设主按钮禁用并在 tooltip 说明原因；精确同路径重复内联拒绝，拼写不同而 realpath 相同的重复仍由宿主 `federation-invalid-members` 兜底；有流程占用时空注册表不再是死路——纯文件夹亦可凑满成员集）；标题默认为 basename 以 `' + '` 连接（用户输入前随成员集合重算），少于两名成员时确认钮禁用并在旁提示，业务失败内联展示而面板保持打开供重试。开关关闭时隐藏全部行与动作。

侧边栏会话树为每个持久化联邦渲染一个可折叠分组，置顶于工作区分组之前：组头为叠层文件夹行（成员 tooltip、「N 个目录」文字说明、「已失效」标记，以及含重命名——对联邦标题集做重名校验——与删除的悬停菜单；删除确认对话框言明非破坏性语义——文件夹、工作区与会话记录保留，现有联合会话继续可用，仅停止新建认领）。点击组头展开/收起；其认领的会话以普通会话行列于组内，点击进入与其他会话是同一 open 手势，空联邦展开时显示「暂无会话」而非无响应点击。归因精确：会话 cwd 为联邦主成员且 additionalRoots 与其余成员逐项相等才归入联邦组——同 cwd 不同根仍是主工作区的普通会话，且已归因会话绝不在主工作区分组下重复列出。分组不受灰度开关约束：开关关闭前创建的联邦仍可管理。列表行标记存在性探针缺失：宿主 list 处理器对成员路径做 stat 并返回 `missingMembers`（联邦）／`missing`（工作区），成员目录消失即在菜单行、组头与 tooltip 中显示「已失效」标记；创建面板将此类工作区锁定在成员集之外并内联说明原因。未探针的投影（字段缺省）表示「未知」，不渲染标记。
...
- **联邦成员创建后不可变**：重命名与删除是仅有的管理动词且没有重排席位（v1 顺序即创建序）；联邦组内会话按最近更新排列、无手动排序席位（联邦组内会话行不可拖拽），会话中途增删根在设计上即不在范围内。
- **缺失成员标记随基线刷新落地**：存在性探针只在 list 处理器运行，目录在会话中途消失要等下一次基线拉取才被标记；在此之前行不带标记渲染。
