...
```sh
node runner.js --workspace <dir> [--writable-root <dir>]... --temp <dir> --mode <read-only|workspace-write> [--write-sid <S-1-4-…> --temp-write-sid <S-1-4-…>] -- <argv...>
```

Each repeatable `--writable-root` is one federation member root: it must be an existing directory distinct from the primary and from its siblings, and — like the primary — it may not contain the private temp root (checked per member before anything spawns). Both ends derive every identity from its owning path (`workspaceWriteSid`), so the additional roots carry no SID flags on the wire; under `workspace-write` with `--write-sid`, each member's standing ACE materializes through that derived identity and the restricted token's workspace-write list carries one workspace SID per member. The direct `AclSandbox` API mirrors this with `additionalWriteSids`, paired positionally with `writableDirs`. Read-only rejects every extra root.
...
- `workspace-write` (logon SID, Everyone, one workspace SID per granted root, temp SID): the workspace — and every federation member root — plus the session's PRIVATE temp subdirectory carry separate Write grants (each member under its own path-derived identity); other ACL-addressable writes are denied except for the documented Everyone and hard-link boundaries.
