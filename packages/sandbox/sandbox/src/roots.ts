 ...
 * policy's named roots — the workspace root first, then any additional
 * roots — plus the platform temp areas", and this module is that meaning's
 * one home. The Seatbelt profile
 ...
 * `workspace-write` allows the policy's named roots — the workspace root
 * first, then any `additionalRoots` in order — plus the host `/tmp` and the
...
export function writableRoots(policy: SandboxExecutionPolicy): string[] {
  ...
  const named = [policy.workspaceRoot, ...(policy.additionalRoots ?? [])]
  return [...new Set([...named, '/tmp', tmpdir()].map(canonicalPath))]
}
